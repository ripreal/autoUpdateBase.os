...
Функция Инициализировать()
    СЕРВЕР = "SRVAPP1CAGRO01";
    БАЗЫ = Новый Массив;
    БАЗЫ.Добавить("ИМЯ ОБНОВЛЯЕМОЙ БАЗЫ");
    ПОЛЬЗОВАТЕЛЬ = "ПОЛЬЗОВАТЕЛЬ ОБНОВЛЯЕМОЙ БАЗЫ";
    ПАРОЛЬ = "ПАРОЛЬ ПОЛЬЗОВАТЕЛЯ ОБНОВЛЯЕМОЙ БАЗЫ";
    ПЛАТФОРМА_ВЕРСИЯ = "ВЕРСИЯ ПЛАТФОРМЫ"; // необязательность, если закомментить, то будет браться последняя версия
    ХРАНИЛИЩЕ_ПУТЬ = "ПУТЬ К КАТАЛОГУ ХРАНИЛИЩА КОНФИГУРАЦИИ";
    ХРАНИЛИЩЕ_ПОЛЬЗОВАТЕЛЬ = "ПОЛЬЗОВАТЕЛЬ ХРАНИЛИЩА";
...
Процедура ОбновитьИБ()
        ...
        Лог.Информация(Строка(ТекущаяДата()) + " Запуск клиента...");
        //Конфигуратор.ЗапуститьВРежимеПредприятия(); //TODO: в этом месте код блокируется, нужен запуск с особым ключом
        Лог.Информация("ОК");
...
Процедура ОтправитьПисьмо(Тема, ТекстHTML, Вложения = Неопределено)
        ...
        Сообщение.Получатели.Добавить("receiver@company.ru");
        Сообщение.ОбратныйАдрес.Добавить("sender@company.ru");
        Сообщение.Отправитель = "sender@company.ru";
        Сообщение.Тема        = Тема;
